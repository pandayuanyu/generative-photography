...
from genphoto.data.dataset import CameraColorTemperature
...
def main(name: str,
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    camera_encoder.requires_grad_(True)
    ...
    encoder_trainable_params = list(filter(lambda p: p.requires_grad, camera_encoder.parameters()))
    encoder_trainable_param_names = [p[0] for p in
                                     filter(lambda p: p[1].requires_grad, camera_encoder.named_parameters())]
    ...
    train_dataset = CameraColorTemperature(**train_data)
    ...
    validation_dataset = CameraColorTemperature(**validation_data)
        ...
        for step in range(trained_iterations, len(train_dataloader)):
            ...
            with torch.cuda.amp.autocast(enabled=mixed_precision_training):
                model_pred = camera_adaptor(noisy_latents,
                                          ...
                                          camera_embedding=camera_embedding)  # [b c f h w]
            ...
            if mixed_precision_training:
                ...
                torch.nn.utils.clip_grad_norm_(filter(lambda p: p.requires_grad, camera_adaptor.parameters()),
                                               max_grad_norm)
                """ <<< gradient clipping <<< """
            ...
            if is_main_process and (
                ...
                for idx, validation_batch in enumerate(validation_data_iter):
                    camera_embedding = validation_batch['camera_embedding'].to(device=unet.device)
                    ...
                    if 'clip_name' in validation_batch:
                        sample_save_path = f"{output_dir}/samples/sample-{global_step}/{validation_batch['clip_name'][0]}_sample.gif"
                        reference_save_path = f"{output_dir}/samples/sample-{global_step}/{validation_batch['clip_name'][0]}_reference.gif"
                    else:
                        sample_save_path = f"{output_dir}/samples/sample-{global_step}/{idx}_sample.gif"
                        reference_save_path = f"{output_dir}/samples/sample-{global_step}/{idx}_reference.gif"
